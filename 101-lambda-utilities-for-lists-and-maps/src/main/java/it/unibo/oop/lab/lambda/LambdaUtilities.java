...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        ...
        list.forEach(elem -> results.add(Optional.of(elem)
                                    .filter(pre))
                                );
        return results;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(elem -> results.merge(op.apply(elem), 
                    new HashSet<>(Set.of(elem)),
                    (oldSet, newSet) -> { //is know that the enter parameters of the lambda are: the old and the new value
                                        oldSet.add(elem);
                                        return oldSet;
                    })
        );
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((key, optValue) -> {
            result.put(key, optValue.orElse(def.get())); });
        return result;
    }
